...
mod_cvd_ps <- coxph(Surv(time, cvd) ~ indager + indsex + smoke_cont + chol + htn_cont + diabetes_cont + strata(chip), data = prior_stat, x = TRUE)
summary(mod_cvd_ps)

fit <- survfit(mod_cvd_ps)
ggs <- ggsurvplot(fit, data = prior_stat, conf.int = TRUE, ylim=c(0.8, 1), xlim=c(2,9), break.time.by = 1)
ggs

res.ggs <- surv_summary(fit, data = prior_stat)

ggs_cu_i <- ggsurvplot(fit, data = prior_stat, conf.int = TRUE, risk.table = TRUE, risk.table.col = "strata", fun = "event", 
                       xlim=c(2,9), 
                       break.time.by = 1,
