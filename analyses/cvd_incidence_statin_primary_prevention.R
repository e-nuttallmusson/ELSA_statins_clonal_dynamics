...
cases <- read_xlsx("Waves 2,4,6,8,9 Lab IDs and Idauniq -Chip cases.xlsx")
...
#wave 2 chip cases
wave_2_chip_cases <- cases %>% dplyr::filter(elsa_wave == 2)
...
match_obj <- matchit(chip_fac ~ indager + indsex + smoke_cont + chol + htn_cont + diabetes_cont, 
                     data = prior_stat_fac,
                     method = "nearest",
                     distance = "glm",
                     ratio = 1,
...
plot.summary.matchit(match_obj)

prior_stat_fac_matched <- get_matches(match_obj)

#add in age, sex and cholesterol as covariates


mod_cvd_ps <- coxph(Surv(time, cvd) ~ indager + indsex + chol + strata(chip), data = prior_stat_fac_matched, x = TRUE)
summary(mod_cvd_ps)

coefficient <- coef(mod_cvd_ps)
hazard_ratio <- exp(coefficient)

fit <- survfit(mod_cvd_ps)

ggs_cu_i <- ggsurvplot(fit, data = prior_stat_fac_matched, conf.int = TRUE, risk.table = TRUE, risk.table.col = "strata", fun = "event", 
                       xlim=c(2,9), 
                       break.time.by = 1,
                       legend.labs = c("No CHIP", "CHIP"),
                       ylab = "Cumulative CVD incidence",
                       xlab = "ELSA wave")
ggs_cu_i

ph_check <- cox.zph(mod_cvd_ps)
