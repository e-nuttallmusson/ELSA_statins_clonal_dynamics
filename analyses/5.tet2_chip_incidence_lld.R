...
#script to analyse association between lld and incidence tet2 chip
...
#wave_8_ctls$elsa_wave <- 8 - none
wave_9_ctls$elsa_wave <- 9

#recombine
tet2_chip_repeats_ctlwave <- do.call("rbind", list(wave_2_ctls, wave_4_ctls, wave_6_ctls, wave_8_ctls, wave_9_ctls))
...
#1 individual loses detectable tet2 
lose_clones <- c([censored])
incident_tet2_simp <- incident_tet2_simp %>% dplyr::filter(!idauniq %in% lose_clones)
...
fisher.test(tet2_incidence_data$outcome, tet2_incidence_data$smoke)
...
sd(tet2_incidence_cases$follow_up, na.rm = TRUE)
sd(tet2_incidence_controls$follow_up, na.rm = TRUE)
