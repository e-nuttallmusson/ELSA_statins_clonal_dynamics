...
chip_variants <- read.table("anon_chip_variant_data_clean_asxl1_fixed.txt", header = TRUE, sep = "\t")

#remove all TET2 Q1274L - present in > 20 and not associated with age

chip_variants <- chip_variants %>% dplyr::filter(NonsynOI != "Q1274L")
...
#remove cases of chip identified in pilot using differing dna conc and different ifc format
...
#replace vaf with 0.01 for relevant control waves as placeholder
...
for (i in (4:23)) {
  chip_cases_no_list[,i] <- unlist(chip_cases_no_list[,i])
}
...
#create key value pairs of bnf drug codes
...
setwd("N:/My Documents/ELSA_data/leukaemia_letter_statins/revisions")
save.image("integrated_longitudinal_variants_and_late_wave_comorbs_drugs.RData")

#640 late wave controls, 1983 rows of late wave cases
