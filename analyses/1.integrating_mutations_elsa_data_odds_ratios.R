#script to integrate filtered variants with elsa data for comorbidities odds ratios
...
chip_variants <- read.table("included_variants_after_flags_reviewed.txt", header = TRUE, sep = "\t")


#now integrate mutation data with idauniqs and late wave comorbidities
...
#same wave sample identified as case and control due to pilot optimisation - can't be sure what outcome is
...
library(purrr)
chip_cases_list_clean <- chip_cases_list %>%
  mutate(across(
    where(is.list),
    ~ map(.x, function(vals) {
      # Remove NAs
      ...
      # If numeric
      if (all(map_lgl(vals, is.numeric))) {
        return(mean(as.numeric(vals)))
      }
      
      # Otherwise (non-numeric): return first unique
      return(unique(vals)[[1]])
    })
  ))
...
chip_cases_clean <- rbind(chip_cases_no_list, chip_cases_list_clean)
chip_cases_clean_simp <- subset(chip_cases_clean, select = c(idauniq, Gene.refGene, NonsynOI, AF_2, AF_4, AF_6, AF_8, AF_9, Sample_2, Sample_4, Sample_6, Sample_8, Sample_9))

chip_cases_clean_simp <- chip_cases_clean_simp %>%
  mutate(across(everything(),
                ~ map(.x, function(val) {
                  if(is.null(val)) return(NA)
                  if(is.character(val) && val == "NA") return(NA)
                  return(val)
                })
  ))
...
#unlist all columnes
chip_cases_clean_simp <- chip_cases_clean_simp %>%
  mutate(across(everything(), ~sapply(.x, function(val) if(is.null(val)) NA else val)))
...
#drop those with blood disorder
chip_cases_comorb_drugs_w8_9 <- chip_cases_comorb_drugs_w8_9 %>% dplyr::filter(blood_dis == 0)

#write this out for regression analyses
setwd("N:/My Documents/ELSA_data/leukaemia_letter_statins/revisions_2/integrate_variants_with_elsa_data")
write.table(chip_cases_comorb_drugs_w8_9, "chip_cases_comorb_drugs_w8_9.txt", row.names = FALSE, quote = FALSE, sep = "\t")


#now need to get comorbidities for controls
controls_8_9 <- rbind(controls_w8, controls_w9)
controls_8_9_simp <- subset(controls_8_9, select = c(idauniq, Sample, elsa_wave))

late_wave_controls_comorb <- merge(controls_8_9_simp, late_wave_comorbs, by = "idauniq", all.x = TRUE)

#add in blood results
late_wave_controls_comorb_bloods <- merge(late_wave_controls_comorb, wave_8_9_bloods, by = "idauniq", all.x = TRUE)

#drop those with blood disorder
late_wave_controls_comorb_bloods <- late_wave_controls_comorb_bloods %>% dplyr::filter(blood_dis == 0)


#now save these data - all wave 8 and 9 cases and controls with most recent comorbidities and wave 8 and 9 drugs

save.image("integrated_longitudinal_variants_and_late_wave_comorbs_drugs_revisions2.RData")


#now just take idauniqs, comorbs and blood parameters for cases and controls, on which to do matching
late_wave_cases_for_matching <- subset(chip_cases_comorb_drugs_w8_9, select = c(idauniq, indsex, indager, current_smoker, inflam_dis, cancer, diabetes, ihd, heart_dis, stroke, osteoporosis, asthma, lung_disease, htn, chol))
#remove duplicates in case of multiple mutations
late_wave_cases_for_matching_dedup <- distinct(late_wave_cases_for_matching)


late_wave_controls_for_matching <- subset(late_wave_controls_comorb_bloods, select = c(idauniq, indsex, indager, current_smoker, inflam_dis, cancer, diabetes, ihd, heart_dis, stroke, osteoporosis, asthma, lung_disease, htn, chol))

######################rbind these and add binary variable for case and control

late_wave_cases_for_matching_dedup$status_num <- 1
late_wave_controls_for_matching$status_num <- 0

cases_controls_for_matching <- rbind(late_wave_cases_for_matching_dedup, late_wave_controls_for_matching)

cases_controls_for_matching$status <- ifelse(cases_controls_for_matching$status_num == 1, "chip", "control")

#replace age -7 with 90

cases_controls_for_matching$indager <- ifelse(cases_controls_for_matching$indager == -7, 90, cases_controls_for_matching$indager)

#matching for age, sex and smoking status - can do this for htn, diabetes, osteoporosis, other lung disease, asthma
library(MatchIt)
library(epitools)

m_out_age_sex_smoking <- matchit(status_num ~ indager + indsex + current_smoker, data = cases_controls_for_matching, method = "nearest", distance = "glm")
m_data_age_sex_smoking <- match.data(m_out_age_sex_smoking)

#htn 
table(m_data_age_sex_smoking$status, m_data_age_sex_smoking$htn)
oddsratio(as.matrix(table(m_data_age_sex_smoking$htn, m_data_age_sex_smoking$status)), rev = "c")

#diabetes 
oddsratio(as.matrix(table(m_data_age_sex_smoking$diabetes, m_data_age_sex_smoking$status)), rev = "c")

#osteoporosis 
oddsratio(as.matrix(table(m_data_age_sex_smoking$osteoporosis, m_data_age_sex_smoking$status)), rev = "c")

#asthma 
oddsratio(as.matrix(table(m_data_age_sex_smoking$asthma, m_data_age_sex_smoking$status)), rev = "c")

#other lung disease 
oddsratio(as.matrix(table(m_data_age_sex_smoking$lung_disease, m_data_age_sex_smoking$status)), rev = "c")


#matching for age, sex, smoking status, htn, diabetes - for ihd, other heart disease, stroke 
#impute choelsterol using cart method in mice
library(mice)
cases_controls_for_matching$chol_imputed <- complete(mice(cases_controls_for_matching, method = "cart"))$chol


m_out_age_sex_smoking_plus <- matchit(status_num ~ indager + indsex + current_smoker + diabetes + chol_imputed + htn, data = cases_controls_for_matching, method = "nearest", distance = "glm")
m_data_age_sex_smoking_plus <- match.data(m_out_age_sex_smoking_plus)

#ihd 
oddsratio(as.matrix(table(m_data_age_sex_smoking_plus$ihd, m_data_age_sex_smoking_plus$status)), rev = "c")

#other heart disease 
oddsratio(as.matrix(table(m_data_age_sex_smoking_plus$heart_dis, m_data_age_sex_smoking_plus$status)), rev = "c")

#stroke  
oddsratio(as.matrix(table(m_data_age_sex_smoking_plus$stroke, m_data_age_sex_smoking_plus$status)), rev = "c")
