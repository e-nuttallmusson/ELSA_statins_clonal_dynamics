...
#one individual loses detectable dnmt3a clone - remove.
incident_dnmt3a_simp <- incident_dnmt3a_simp %>% dplyr::filter(idauniq != "[censored]")
...
chisq.test(dnmt3a_incidence_data$outcome, dnmt3a_incidence_data$htn)
...
fit <- survfit(formula = mod_dnmt3a_incidence ~ cvd + strata(statin), data = dnmt3a_incidence_data)
...
dnmt3a_coxph_model <- coxph(Surv(time_in_years, status) ~ statin, data = dnmt3a_incidence_data)
cox.zph(dnmt3a_coxph_model)

fit <- survfit(formula = mod_dnmt3a_incidence ~ statin, data = dnmt3a_incidence_data)


ggsurvplot(fit, data = dnmt3a_incidence_data, pval = TRUE, risk.table = TRUE,
           fun = "event",
           conf.int = TRUE,
           break.time.by = 1,
           ylab = "DNMT3A CHIP cumulative events",
