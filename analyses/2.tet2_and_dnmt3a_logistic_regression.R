#script for logistic regression analysis of effect of statins on high vaf vs low vaf DNMT3A and TET2 CHIP
...
#3 individuals without cholesterol value - multiple imputation
chip_tet2_mut_no_dups$chol_imputed <- complete(mice(chip_tet2_mut_no_dups, method = "cart"))$chol


#remove 2 samples who are early wave cases with no detectable variants in w8/9
...
sd(chip_tet2_mut_no_dups_fbc$chol, na.rm = TRUE)
...
#1 individual without statin info - exclude
chip_tet2_mut_no_dups_complete <- chip_tet2_mut_no_dups %>% dplyr::filter(!is.na(chip_tet2_mut_no_dups$statins))

tet2_statin_model <- glm(vaf_cat ~ indager + indsex + cvd + statins + htn + chol_imputed + antiplatelets + bb + RAAS, family = binomial(link = "logit"), data = chip_tet2_mut_no_dups_complete)
...
logit_check <- subset(chip_tet2_mut_no_dups_complete, select = c(indager, chol_imputed))
predictors <- colnames(logit_check)

#save predicted probabilities
logit_check$probabilities <- tet2_statin_model$fitted.values
...
#age and sex adjusted univariate analyses
#statins
statin_uni_model <- glm(vaf_cat ~ indager + indsex + statins, family = binomial(link = "logit"), data = chip_tet2_mut_no_dups_complete)
summary(statin_uni_model)
exp(cbind(OR=coef(statin_uni_model), confint(statin_uni_model)))

#antiplatelets
antiplt_uni_model <- glm(vaf_cat ~ indager + indsex + antiplatelets, family = binomial(link = "logit"), data = chip_tet2_mut_no_dups_complete)
...
#4 individuals without cholesterol value - multiple imputation
library(mice)
chip_dnmt3a_mut_no_dups$chol_imputed <- complete(mice(chip_dnmt3a_mut_no_dups, method = "cart"))$chol



#remove 7 samples who are early wave cases with no detectable variants in w8/9
summary(chip_dnmt3a_mut_no_dups$recent_vaf)
chip_dnmt3a_mut_no_dups <- chip_dnmt3a_mut_no_dups %>% dplyr::filter(recent_vaf >= 0.02)
...
table(chip_dnmt3a_mut_no_dups$heart_dis)
...
summary(chip_dnmt3a_mut_no_dups_fbc$chol_imputed)
sd(chip_dnmt3a_mut_no_dups_fbc$chol_imputed)

##################logistic regression

dnmt3a_statin_model <- glm(vaf_cat ~ indager + indsex + cvd + statins + htn + chol_imputed + antiplatelets + bb + RAAS, family = binomial(link = "logit"), data = chip_dnmt3a_mut_no_dups)
...
logit_check <- subset(chip_dnmt3a_mut_no_dups, select = c(indager, chol_imputed))
predictors <- colnames(logit_check)

#save predicted probabilities
logit_check$probabilities <- dnmt3a_statin_model$fitted.values
...
#looks like indager has sigmoid relationship with logit
#try natural splines


dnmt3a_statin_model_ns3 <- glm(vaf_cat ~ ns(indager, df = 3) + indsex + cvd + statins + htn + chol_imputed + antiplatelets + bb + RAAS, family = binomial(link = "logit"), data = chip_dnmt3a_mut_no_dups)
summary(dnmt3a_statin_model_ns3)

anova(dnmt3a_statin_model, dnmt3a_statin_model_ns3, test = "LRT")

#non significant therefore linear indager sufficient 
