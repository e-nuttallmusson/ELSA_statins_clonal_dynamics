...
#5 individuals without cholesterol value - multiple imputation
...
#statins
statin_uni_model <- glm(vaf_cat ~ indager + statins, family = binomial(link = "logit"), data = chip_tet2_mut_no_dups_complete)
summary(statin_uni_model)
exp(cbind(OR=coef(statin_uni_model), confint(statin_uni_model)))

#antiplatelets
antiplt_uni_model <- glm(vaf_cat ~ indager + antiplatelets, family = binomial(link = "logit"), data = chip_tet2_mut_no_dups_complete)
...
#1 individual without statin info - exclude
chip_dnmt3a_mut_no_dups_complete <- chip_dnmt3a_mut_no_dups %>% dplyr::filter(!is.na(chip_dnmt3a_mut_no_dups$statins))

dnmt3a_statin_model <- glm(vaf_cat ~ indager + indsex + cvd + statins + htn + chol_imputed + antiplatelets + bb + RAAS, family = binomial(link = "logit"), data = chip_dnmt3a_mut_no_dups_complete)
...
logit_check <- subset(chip_dnmt3a_mut_no_dups_complete, select = c(indager, chol_imputed))
predictors <- colnames(logit_check)
