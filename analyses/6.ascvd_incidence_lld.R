#script to repeat statin primary prevention cvd cumulative incidence 
...
chip_vars <- read.table("included_variants_after_flags_reviewed.txt", header = TRUE, sep = "\t")
...
cases_post_filtering <- cases %>% dplyr::filter(Sample %in% chip_vars$Sample)
#now drop unnecessary variables

cases_simp <- subset(cases_post_filtering, select=-c(bldrec, consn, CONBST))
...
#wave 2 chip cases
wave_2_chip_cases <- cases_post_filtering %>% dplyr::filter(elsa_wave == 2)
...
prior_stat_fac$chip_status <- ifelse(prior_stat_fac$chip == 1, "Control", "CHIP")
